...
CPU: {
    ...
    VECTORS: {
        ...
        // Usage: sta CPU.VECTORS.NMI     (lo byte → $FFFA)
        //        sta CPU.VECTORS.NMI + 1 (hi byte → $FFFB)
        .label NMI   = $FFFA    // non-maskable interrupt handler pointer
        .label RESET = $FFFC    // power-on / hard-reset handler pointer
        .label IRQ   = $FFFE    // maskable interrupt / BRK handler pointer
    }
